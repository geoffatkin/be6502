...
ace_dlm = ace_base + $01    ; divisor latch MSB (when DLAB = 1)
    ...
    .if serial_clock == 3072
; Baud rate = 3.072 MHz/(16 * divisor); set ace_dlm=0, ace_dll as below.
ACE_2400_BAUD   = 80
ACE_3600_BAUD   = 53
ACE_4800_BAUD   = 40
ACE_9600_BAUD   = 20
...
ACE_8N1                 = ACE_8_BITS | ACE_1_STOP | ACE_NO_PARITY
ACE_7E1                 = ACE_7_BITS | ACE_1_STOP | ACE_EVEN_PARITY
...
ACE_DCD                 = %10000000 ; data carrier detect = MSR7 (loop mode: equals MCR3)
    ...
    .struct ace_config_block
DLL .byte   ACE_38400_BAUD
LCR .byte   ACE_8_BITS | ACE_1_STOP | ACE_NO_PARITY
IER .byte   ACE_ENABLE_RECV_INTERRUPT
FCR .byte   ACE_FIFO_ENABLE | ACE_RECV_FIFO_RESET | ACE_XMIT_FIFO_RESET | ACE_RECV_TRIGGER_14
MCR .byte   ACE_AUTO_RTS_AND_CTS | ACE_OUT2
    .endstruct

; Initialize the serial port from the values in ace_config, which must be an ace_config_block.
ace_init:
    lda #ACE_DLAB
    sta ace_lcr
    ldx #ace_config_block.DLL
    lda ace_config,x
    sta ace_dll
    lda #0
    sta ace_dlm
    ldx #ace_config_block.LCR
    lda ace_config,x
    sta ace_lcr
    lda #0
    sta ace_ier
    ldx #ace_config_block.FCR
    lda ace_config,x
    sta ace_fcr
    ldx #ace_config_block.MCR
    lda ace_config,x
    sta ace_mcr
    ldx #ace_config_block.IER
    lda ace_config,x
    sta ace_ier
    rts
